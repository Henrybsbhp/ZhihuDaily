...
struct TopicTableModel {
    ...
    var imageURL: String? = String() //  图像地址（官方 API 使用数组形式。目前暂未有使用多张图片的情形出现，曾见无 images 属性的情况，请在使用中注意 ）
    var id: String? = String() // url 与 share_url 中最后的数字（应为内容的 id）
...
struct NewsContentModel {
    ...
    var title: String? = String() // 新闻标题
    var imageURL: String? = String() //  获得的图片同 最新消息 获得的图片分辨率不同。这里获得的是在文章浏览界面中使用的大图。
    var imageSource: String? = String() //  图片的内容提供方。为了避免被起诉非法使用图片，在显示图片时最好附上其版权信息。
    var body: String? = String() // HTML 格式的新闻
